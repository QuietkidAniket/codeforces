...
void Solve(){
    ...
    {
        ndivs *= (alpha[i]+1);
        ndivs_exp = (ndivs_exp * alpha[i]+1)%(MOD -1);
    }
    cout << ndivs << " ";
    ...
    mint sum = 1ll;
    for(int i =0;i < n; i++){
        sum *= (mint(p[i]).power(alpha[i]+1) - 1) / mint(p[i]-1);
    }
    cout << sum << " ";

    // find the product of divisors 
    
    int exp  = ndivs * modPow();
